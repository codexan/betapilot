...
const ScheduleSlotStep = ({ data, updateData, onSendNow, campaignState, loading, onCampaignSelect }) => {
  ...
  const checkCampaignInvites = async (campaignId) => {
    try {
      console.log('Checking campaign invites for campaign ID:', campaignId);
    ...
    <div className="space-y-8">
        ...
        <div className="p-4 border rounded-lg bg-muted/20">
          ...
          <select
            ...
            onChange={(e) => {
              ...
              if (id) {
                onCampaignSelect?.(id);       // loads campaignData + sets betaProgramId
                checkCampaignInvites(id);     // runs validation immediately
              } else {
      ...
      {validationStatus && (
        <div className={`p-4 rounded-lg border ${
          ...
          {validationStatus.valid && recipients.length > 0 && (
            <div className="mt-2 text-xs">
              Ready to send to {recipients.length} beta testers who received campaign invites.
            </div>
          )}
        </div>
      )}
